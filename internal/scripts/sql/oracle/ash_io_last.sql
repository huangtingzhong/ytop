-- File Name: ash_io_last.sql
-- Purpose: Oracle ASH Io Last
...
select *
  from (SELECT to_char(a.SAMPLE_TIME, 'hh24:mi:ss') stime,
               ...
               trunc((a.DELTA_WRITE_IO_BYTES * 1000000) /
                     (DELTA_TIME * 1024)) write_kpbs,
               row_number() over(partition by a.sample_time order by to_number(DELTA_READ_IO_BYTES) desc) topr,
               row_number() over(partition by a.sample_time order by to_number(DELTA_WRITE_IO_BYTES) desc)  topw
          FROM GV$ACTIVE_SESSION_HISTORY a
         WHERE SAMPLE_TIME >= SYSDATE - 1 / 1440
           and (a.DELTA_READ_IO_REQUESTS > 0 or
               a.DELTA_WRITE_IO_REQUESTS > 0 or a.DELTA_READ_IO_BYTES > 0 or
               a.DELTA_WRITE_IO_BYTES > 0)
