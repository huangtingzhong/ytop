-- File Name: awr_segment_unoptimized_reads.sql
-- Purpose: Oracle AWR Segment Unoptimized Reads
...
col physical_reads for 9999999999999999 heading 'UnOptimized Reads'
col ratio for 99999.99 heading '% of Capture'
/* Formatted on 2013/2/24 22:15:57 (QP5 v5.215.12089.38647) */
SELECT owner,
       tablespace_name,
       object_name,
       subobject_name,
  ...
  FROM (  SELECT n.owner,
            ...
            FROM dba_hist_seg_stat_obj n,
                 ...
                 (SELECT   SUM (physical_read_requests_delta)
                         - SUM (optimized_physical_reads_delta)
                            phydrq
                    FROM dba_hist_seg_stat
                   WHERE     :bid < snap_id
