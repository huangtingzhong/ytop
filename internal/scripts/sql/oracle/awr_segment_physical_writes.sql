-- File Name: awr_segment_physical_writes.sql
-- Purpose: Oracle AWR Segment Physical Writes
...
col physical_writes for 9999999999999999 heading 'Physical Writes'
col ratio for 99999.99 heading '% of Capture'
/* Formatted on 2013/2/24 22:24:44 (QP5 v5.215.12089.38647) */
SELECT owner,
       tablespace_name,
       object_name,
       subobject_name,
  ...
  FROM (  SELECT n.owner,
            ...
            FROM dba_hist_seg_stat_obj n,
                 ...
                 (SELECT SUM (physical_writes_delta) phyw
                    FROM dba_hist_seg_stat
                   WHERE     :bid < snap_id
                         AND snap_id <= :eid              /*AND dbid = :dbid*/
