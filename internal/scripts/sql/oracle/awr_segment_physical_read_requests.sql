-- File Name: awr_segment_physical_read_requests.sql
-- Purpose: Oracle AWR Segment Physical Read Requests
...
col physical_reads for 9999999999999999 heading 'Phys Read Requests'
...
/* Formatted on 2013/2/24 22:10:04 (QP5 v5.215.12089.38647) */
/* Formatted on 2013/2/24 22:12:36 (QP5 v5.215.12089.38647) */
SELECT owner,
       tablespace_name,
       object_name,
       subobject_name,
  ...
  FROM (  SELECT n.owner,
                 ...
                 DECODE (d.phyrq,
                         0, TO_NUMBER (NULL),
                         100 * r.physical_reads / d.phyrq)
                    ratio
            FROM dba_hist_seg_stat_obj n,
                 (  SELECT dataobj#,
                           obj#,
                           dbid,
                           SUM (physical_read_requests_delta) physical_reads
                 ...
                 (SELECT SUM (physical_read_requests_delta) phyrq
                    FROM dba_hist_seg_stat
