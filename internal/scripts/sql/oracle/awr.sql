-- File Name: awr.sql
-- Purpose: Oracle Show AWR load profile metrics by snapshot
...
WITH SAMPLE AS
 (SELECT SNAP_ID,DECODE(STAT_NAME,'user commits','transactions','user rollbacks','transactions','consistent gets','logical reads','db block gets',
   ...
   WHERE STAT_NAME IN ('redo size','user commits','user rollbacks','logons cumulative','consistent gets','db block gets','db block changes',
         'physical writes','physical reads','parse count (hard)','parse count (total)','user calls','execute count','sorts (memory)','sorts (disk)','physical write total IO requests','physical read total IO requests')
...
&_GV_TOTAL  ROUND(SUM(DECODE(NAME, 'physical write total IO requests', PER_SECOND, 5))) IOPS_W,
&_GV_TOTAL  ROUND(SUM(DECODE(NAME, 'physical read total IO requests', PER_SECOND, 0))) IOPS_R,
&_GV_TOTAL  (ROUND(SUM(DECODE(NAME, 'physical write total IO requests', PER_SECOND, 0)))+ROUND(SUM(DECODE(NAME, 'physical read total IO requests', PER_SECOND, 0)))) IOPS,
