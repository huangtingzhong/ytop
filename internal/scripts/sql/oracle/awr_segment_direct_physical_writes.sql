-- File Name: awr_segment_direct_physical_writes.sql
-- Purpose: Oracle AWR Segment Direct Physical Writes
...
/* Formatted on 2013/2/24 22:29:50 (QP5 v5.215.12089.38647) */
SELECT owner,
       tablespace_name,
       object_name,
       subobject_name,
  ...
  FROM (  SELECT n.owner,
                 ...
                 DECODE (d.phywdir,
                         0, TO_NUMBER (NULL),
                         100 * r.physical_writes / d.phywdir)
                    ratio
            FROM dba_hist_seg_stat_obj n,
                 (  SELECT dataobj#,
                           obj#,
                           dbid,
                           SUM (physical_writes_direct_delta) physical_writes
                      FROM dba_hist_seg_stat
                     WHERE     :bid < snap_id
                           AND snap_id <= :eid            /*AND dbid = :dbid*/
                           AND instance_number = :inst_num
                  GROUP BY dataobj#, obj#, dbid) r,
                 (SELECT SUM (physical_writes_direct_delta) phywdir
                    FROM dba_hist_seg_stat
                   WHERE     :bid < snap_id
                         AND snap_id <= :eid              /*AND dbid = :dbid*/
