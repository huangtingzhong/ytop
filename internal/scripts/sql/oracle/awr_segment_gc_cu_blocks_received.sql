-- File Name: awr_segment_gc_cu_blocks_received.sql
-- Purpose: Oracle AWR Segment Gc Cu Blocks Received
...
SELECT owner,
  ...
  FROM (  SELECT n.owner,
                 ...
                 n.subobject_name,
                 n.object_type,
                 r.GC_CU_BLOCKS_RECEIVED,
                 ROUND (r.ratio * 100, 2) ratio
            FROM dba_hist_seg_stat_obj n,
                 (  SELECT dataobj#,
                           obj#,
                           dbid,
                           SUM (GC_CU_BLOCKS_RECEIVED_delta) GC_CU_BLOCKS_RECEIVED,
                           ratio_to_report (SUM (GC_CU_BLOCKS_RECEIVED_delta)) OVER () ratio
           ...
           WHERE     n.dataobj# = r.dataobj#
                 AND n.obj# = r.obj#
                 AND n.dbid = r.dbid
                 AND r.GC_CU_BLOCKS_RECEIVED > 0
        ORDER BY r.GC_CU_BLOCKS_RECEIVED DESC,
                 object_name,
                 owner,
                 subobject_name)
 WHERE ROWNUM <= :top_n
