...
-- Purpose: Create SQLMAP from two gv$sql sql_ids; long SQL via stub+UPDATE SQL_MAP$
...
-- Path:
--   1) Prefer CREATE SQLMAP via EXECUTE IMMEDIATE when SQL/DDL within ~32K;
--   2) If over threshold or CREATE fails: short CREATE stub + UPDATE SYS.SQL_MAP$
    ...
    IF v_ddl_ready THEN
      IF NVL(v_ddl_len, 0) > c_max_sql_bytes THEN
        DBMS_OUTPUT.PUT_LINE(
          ...
          ' > ' || c_max_sql_bytes || '); prefer stub+UPDATE path or sqlmap_gen_ddl.sql');
      ELSE
...
BEGIN
  ...
  BEGIN
    ...
    IF v_preview_only THEN
      ...
      IF v_over_limit THEN
        DBMS_OUTPUT.PUT_LINE(
          ...
          ' bytes; real create will use stub+UPDATE SYS.SQL_MAP$ fallback');
      END IF;
    ...
    -- 7a. Over threshold: stub + UPDATE SYS.SQL_MAP$ (skip full CREATE)
    IF v_over_limit THEN
      DBMS_OUTPUT.PUT_LINE(
        'INFO: SQL/DDL > ' || c_max_sql_bytes ||
        ' bytes; using stub+UPDATE SYS.SQL_MAP$ path');
      BEGIN
        create_via_sql_map_patch;
      EXCEPTION
        WHEN OTHERS THEN
          print_fail('SQLMAP stub+UPDATE SQL_MAP$ failed');
      END;
      RETURN;
    END IF;

    -- 7b. Normal path: CREATE; on failure fall back to stub+UPDATE
    BEGIN
      EXECUTE IMMEDIATE v_ddl;
      report_success('CREATE SQLMAP');
    EXCEPTION
      WHEN OTHERS THEN
        v_create_err := SQLERRM;
        DBMS_OUTPUT.PUT_LINE('CREATE SQLMAP failed: ' || v_create_err);
        DBMS_OUTPUT.PUT_LINE('INFO: falling back to stub+UPDATE SYS.SQL_MAP$');
        BEGIN
          create_via_sql_map_patch;
        EXCEPTION
          WHEN OTHERS THEN
            print_fail('CREATE failed; stub+UPDATE SQL_MAP$ also failed');
        END;
    END;
  EXCEPTION
    WHEN OTHERS THEN
