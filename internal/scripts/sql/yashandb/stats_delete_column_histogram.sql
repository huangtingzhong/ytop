-- File Name: stats_delete_column_histogram.sql
-- Purpose: Delete column histogram only (keep basic column stats)
-- Created: 20260801 by huangtingzhong
--
-- Flow: list columns; if column_name set, check table/column then delete HISTOGRAM then list again
-- Note: ytop prompts all vars before run; leave column_name empty to list only
-- Note: YashanDB uses type=>'HISTOGRAM' (not Oracle col_stat_type)
...
ACCEPT column_name PROMPT 'Enter column_name to delete HISTOGRAM (empty=list only): '
