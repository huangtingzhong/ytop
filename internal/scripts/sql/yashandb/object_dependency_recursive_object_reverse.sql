-- File Name: object_dependency_recursive_object_reverse.sql
-- Purpose: Recursive reverse dependency tree (who depends on this object)
-- Created: 20260801 by huangtingzhong
--
-- Params: &&owner, &&object_name, &&object_type (required)
-- Note: nested PROCEDURE (no CREATE PROCEDURE); Oracle uses schema recur_object_reverse
...
ACCEPT object_name PROMPT 'Enter object name (e.g. DBA_TABLES): '
...
DECLARE
  ...
  PROCEDURE recur_object_reverse(
    p_owner  VARCHAR2,
    p_name   VARCHAR2,
    p_type   VARCHAR2,
    p_level  NUMBER
  ...
  BEGIN
    ...
    FOR c_rec IN (
      ...
      recur_object_reverse(c_rec.owner, c_rec.name, c_rec.type, p_level + 1);
    END LOOP;
  END recur_object_reverse;
BEGIN
  IF v_owner IS NULL OR v_object_name IS NULL OR v_object_type IS NULL THEN
    RAISE_APPLICATION_ERROR(30602, 'owner, object_name and object_type are required');
  END IF;
  recur_object_reverse(v_owner, v_object_name, v_object_type, 0);
END;
/
