-- File Name: lock.sql
-- Purpose: YashanDB row/tx lock holder and waiter (single-line <= ~300 chars)
-- Created: 20260615  by  huangtingzhong
-- Note: WaitTime unit adaptive MS/S/M/H, max 5 chars.
...
WITH base AS (
  SELECT a.lmode,
   ...
   WHERE b.type != 'BACKGROUND'
     ...
     AND (a.lmode IN ('ROW', 'TX', 'TS') OR a.request IN ('ROW', 'TX', 'TS'))
)
