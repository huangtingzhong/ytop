-- File Name: awr_sql_by_buffer_gets_by_day.sql
-- Purpose: YashanDB AWR top SQL by buffer gets per day
...
ranked AS (
    SELECT p.snap_day,
           ...
           ROW_NUMBER() OVER (PARTITION BY p.snap_day ORDER BY p.bget DESC NULLS LAST, p.SQL_ID, p.module) AS rn
    FROM per_day_agg p
    JOIN totals t ON t.snap_day = p.snap_day
    WHERE NVL(p.bget, 0) > 0
),
top10 AS (
...
SELECT t.snap_day,
       TO_CHAR(ROW_NUMBER() OVER (PARTITION BY t.snap_day ORDER BY t.bget DESC NULLS LAST, t.SQL_ID, t.module)) AS rn,
       ...
       t.sql_text
FROM top10 t
ORDER BY t.snap_day DESC, t.bget DESC NULLS LAST, t.SQL_ID, t.module
/
