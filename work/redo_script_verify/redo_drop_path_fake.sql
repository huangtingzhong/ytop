...
-- Usage (one parameter /tmp/no_such_redo_dir):
...
DECLARE
  p_target      VARCHAR2(512) := TRIM('/tmp/no_such_redo_dir');
...
BEGIN
  IF p_target IS NULL OR LENGTH(p_target) = 0 OR p_target = '/tmp/no_such_redo_dir' THEN
    RAISE_APPLICATION_ERROR(-20001, 'Missing /tmp/no_such_redo_dir: pass redo group id (digits) or file/directory path');
  END IF;
