...
public class BindRefresh {
    ...
    public boolean shouldReExport(JdbcSession session, Path outdir, String sqlId) throws SQLException {
        ...
        Integer capFilled = captureFilledCount(session.getConnection(), sqlId);
        if (capFilled == null) {
            return true;
        }
    ...
    private static int parseChild(String fromMeta, String dirName) {
        try {
            if (!fromMeta.isEmpty()) {
                return Integer.parseInt(fromMeta);
            }
        } catch (NumberFormatException ignored) {
        }
        int c = dirName.indexOf("__c");
        if (c >= 0) {
            String rest = dirName.substring(c + 3);
            int i = rest.indexOf("__i");
            if (i >= 0) {
