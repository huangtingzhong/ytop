...
public final class SqlMapDdl {
        ...
        try {
            ...
            try {
                ...
                if (mapName == null || mapName.trim().isEmpty()) {
                    String ts = new SimpleDateFormat("yyyyMMddHHmmss", Locale.ROOT).format(new Date());
                    if (src.sqlId != null && !src.sqlId.isEmpty()) {
                        mapName = "map_" + src.sqlId + "_" + ts;
                    } else {
                        mapName = "map_file_" + ts;
                    }
                }
    ...
    private static boolean dropConflictsByCompare(Connection c, String srcText, DualLogger log) {
        ...
        // 优先 DBMS_LOB.COMPARE (CLOB 对 CLOB)
        try (PreparedStatement ps = c.prepareStatement(
            ...
            ps.setClob(2, clob);
            boolean any = dropNamesFromRs(c, ps, log);
            try {
                clob.free();
        ...
        try (PreparedStatement ps = c.prepareStatement(
                "SELECT name FROM SYS.SQL_MAP$ WHERE DBMS_LOB.GETLENGTH(sql_text) = ?"
                        + " AND DBMS_LOB.SUBSTR(sql_text, ?, 1) = ?")) {
            ps.setInt(1, srcLen);
            ps.setInt(2, Math.min(srcLen, 32767));
            if (srcLen <= 32000) {
                ps.setString(3, srcText);
            } else {
                Clob clob = c.createClob();
                clob.setString(1, srcText);
                ps.setClob(3, clob);
            }
            return dropNamesFromRs(c, ps, log);
        } catch (Exception e) {
            log.logInfo("[WARN] conflict scan (substr): " + e.getMessage());
            return false;
        }
    }
    ...
    private static java.util.List<String[]> loadBinds(Connection c, SqlMapArgs a, String sqlId,
        ...
        if (sqlId != null && !sqlId.isEmpty()) {
            return SqlLookup.toReplayRows(
                    SqlLookup.loadBindsBySqlId(c, sqlId, SqlMapIo.warn(log)));
        }
